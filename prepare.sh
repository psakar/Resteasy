...
# export TEST_GUICE=0 # if value is 1 then install resteasy-guice related artifacts which are not avaiable in AS repo
#   export GUICE_VERSION=3.0-redhat-2
#   export GOOGLE_VERSION=5-redhat-2
# export TEST_UNSUPPORTED=0 #if value is 1 then install artifacts resteasy-fastinfoset-provider, resteasy-jdk-http, resteasy-netty which are not avaiable in AS repo
#
